...
const bookingStatusSchema = z.enum(["pending", "followed_up", "deal", "canceled"])
...
const createMcBookingSchema = z.object({
  clientName: z.string().trim().min(2, "Nama klien wajib diisi minimal 2 karakter."),
...
const createMcServiceSchema = z.object({
  title: z.string().trim().min(2, "Nama layanan wajib diisi minimal 2 karakter."),
  badgeLabel: z.string().trim().optional(),
  badgeVariant: z.string().trim().optional(),
  shortDescription: z.string().trim().min(10, "Deskripsi layanan terlalu pendek."),
  features: z
...
const createFaqSchema = z.object({
  question: z.string().trim().min(3, "Pertanyaan wajib diisi minimal 3 karakter."),
...
const updateWebsiteSettingsSchema = z.object({
  ...
  mcWhatsappTemplate: z.string().trim().min(10, "Template pesan MC terlalu pendek."),
...
export type AdminInvitationOrderStatus = z.infer<typeof invitationOrderStatusSchema>
...
export type InvitationTemplateOption = z.infer<typeof invitationTemplateOptionSchema>
...
export async function listInvitationTemplateOptions(): Promise<InvitationTemplateOption[]> {
  const session = await getAdminSession()
...
export async function deleteFaq(
  ...
  const supabase = await createClient()
  const { data, error } = await supabase.from("faqs").delete().eq("id", parsed.data.id).select("id").single()
...
export async function createInvitationTemplate(
  ...
  const previewImageUrl = uploadedPreview?.data.publicUrl ?? normalizeUrl(parsed.data.previewImageUrl)
  ...
  if (error || !data) {
    if (uploadedPreview?.ok) {
      await supabase.storage.from("invitation-template").remove([uploadedPreview.data.storagePath])
    }
...
export async function createInvitationOrder(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      phone: normalizeOptional(parsed.data.phone),
      event_date: parsed.data.eventDate,
      target_completion_date: normalizeOptional(parsed.data.targetCompletionDate),
